...
def calcADJ(coord, k=8, distanceType='euclidean', pruneTag='NA'):
    ...
    """
    spatialMatrix=coord  #.cpu().numpy() if using a GPU tensor
    nodes=spatialMatrix.shape[0]# Number of spots
    Adj=torch.zeros((nodes,nodes)) # Initialize an empty adjacency matrix

    # Iterate over each spot in the spatial matrix
    for i in np.arange(spatialMatrix.shape[0]):
        tmp=spatialMatrix[i,:].reshape(1,-1)# Get the coordinates of the current spot as a row vector
        distMat = distance.cdist(tmp,spatialMatrix, distanceType)# Compute pairwise distances between the current spot and all others
        if k == 0:
            k = spatialMatrix.shape[0]-1 # If k is 0, set it to all other spots except the current one
        res = distMat.argsort()[:k+1] # Sort distances and get indices of the nearest k spots
        tmpdist = distMat[0,res[0][1:k+1]]  # Extract the distances for the k nearest neighbors
        boundary = np.mean(tmpdist)+np.std(tmpdist) 
        for j in np.arange(1,k+1):
            # No prune
            if pruneTag == 'NA':
                Adj[i][res[0][j]]=1.0 # Set adjacency to 1 if no pruning is applied
            elif pruneTag == 'STD':
            ...
            # Prune: only use nearest neighbor as exact grid: 6 in cityblock, 8 in euclidean
                ...
                if distMat[0,res[0][j]]<=2.0:
                    Adj[i][res[0][j]]=1.0  # Set adjacency to 1 if the distance is within a grid threshold (e.g., 2.0)
    return Adj
