...
class Initiate_dataset(Dataset):
    def __init__(self,patch_path,exp_label_path,orig_path,mode,fold,data_name):
        ...
        self.mode = mode
        self.wsi_ids_lst = [i.split('.')[0] for i in os.listdir(exp_label_path)] #Get all wsi_lst
        self.wsi_ids_lst.sort() #To find the correct order for test sample
        #Define base and train transformation of img
        ...
        print('Loading img...')
        if mode != 'train':
            self.wsi_ids = [self.wsi_ids_lst[fold]] #Get the test sample
            self.test_sample = self.wsi_ids[0] 

        else:
            self.wsi_ids = list(set(self.wsi_ids_lst) - set([self.wsi_ids_lst[fold]]))  #31
        if data_name=='her2':
            self.patch_meta = {wsi_i:[j for j in os.listdir(self.patch_path) if j.split('_')[0] == wsi_i] for wsi_i in self.wsi_ids}#wsi:[patches]
            self.patch_ids = []
            for wsi_i in list(self.patch_meta.keys()):
                self.patch_ids.extend(self.patch_meta[wsi_i]) #extend all patches in self.patch_ids
            print('Loading orig exp...')
            self.exps = {patch_i: torch.Tensor(self.get_exp(patch_i.split('_')[0])[patch_i.split('.')[0]]) for patch_i
                         in
                         self.patch_ids}#patch:tensor(exp)
        elif data_name=='cscc':  
    ...
    def __getitem__(self, item):
        ...
        patch_name = list(self.exps.keys())[item] #patch_path
        patch = self.get_img(patch_name)  #(3,224,224)
        exp = self.exps[patch_name] #label
        patch_orig_path = os.path.join(self.orig_path,patch_name.replace('png','npy')) #Get noise_exp path
    ...
    def get_img(self,img_name):
        ...
        img = torch.Tensor(np.array(img)/255) #transform img array to (0-1)
        img = img.permute(1, 0, 2)  #Keep align with true img. transpose img
        img = img.permute(2,0,1) #Change channels to (3,224,224)
    ...
    def transforms(self,img):
        ...
        if self.mode!='train':
            return my_transforms[0](img) #Only basic transform
        else:
            return my_transforms[1](my_transforms[0](img)) #Add variability to img
...
#Define api to get the loader and test sample
...
if __name__ == '__main__':
    ...
    ref_path = './05-ref.csv'
    predicted_gene_path = './hvg_1000.npy'
    orig_path = '/05-orig_exp/'
